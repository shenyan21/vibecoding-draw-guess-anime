...
- 公网部署：生产模式由同一个 Express 服务托管前端静态文件和 Socket.IO。
...
`npm run dev` 会同时启动前端 Vite 和后端 Socket.IO。Vite 开发服务器已把 `/socket.io` 代理到 `http://localhost:3001`，所以浏览器打开 `http://localhost:5173` 即可联机。
...
局域网测试时，其他设备访问 `http://<开发机局域网IP>:5173`。如果前端需要直连其它后端地址，可设置：

```powershell
$env:VITE_SERVER_URL = 'http://<后端IP或域名>:3001'
npm run dev:client
```

## 公网部署

生产模式下，服务端会在 `NODE_ENV=production` 时托管 `apps/client/dist`，并把非 `/socket.io`、`/health` 路由回退到前端 `index.html`。
...
- 主页：http://<公网IP或域名>/
- 健康检查：http://<公网IP或域名>/health

注意事项：

- `start-ip80.cmd` 从当前项目目录启动，适合临时前台运行。
- `start-server.ps1` 默认部署目录是 `C:\apps\drawandguess`，如项目放在其它目录，需要先修改脚本里的 `$workingDir`、`$serverScript`、日志路径。
- 公网机器需要放行 TCP 80 端口；如果 80 被占用，先运行 `.\stop-server.ps1` 或手动处理占用进程。
- 前端改动后必须重新执行 `npm run build` 再重启生产服务。
- 房间只存在服务端内存中，重启服务会清空所有房间和玩家状态。
...
默认连接 `http://localhost:3001`。连接其它服务时设置 `SERVER_URL`：
...
机器人仅用于流程测试。服务端重启会清空内存房间，需要重新建房并重新启动机器人。
...
`npm run preview` 只预览前端静态产物，不会自动启动后端。完整生产链路请使用 `NODE_ENV=production` 启动 `apps/server`。
...
├─ start-ip80.cmd               Windows 前台公网启动脚本
├─ start-server.ps1             Windows 后台公网启动脚本
├─ stop-server.ps1              停止 80 端口服务
└─ AGENT.md                     Codex 接手说明
```
